...
**Crew `count` (headcount) is a frontend-only multiplier:** salary is PER PERSON and `computeCrewMonthlyTotal` does `salary * count * months/12`, folded into `monthly_crew_eur`; the engine never sees `count`. `count` is in the OpenAPI `CrewMember` schema (optional, default 1) only so it survives zod and round-trips in the stored `crew_breakdown` snapshot. Legacy rows without `count` hydrate to 1 → totals byte-identical.
**Only `countable` roles (Stewardess, Deckhand) carry a count** — owner was explicit: a captain/mate/engineer/chef is always exactly one person, a counter there is meaningless clutter. `roleSupportsCount(role)` (driven by `CREW_POSITIONS[].countable`) gates BOTH the UI stepper AND the math/payload (non-countable roles force count=1 regardless of stored value). To make another role multi-person, flip its `countable` flag — do not special-case in the helpers.
...
## Exit scenario (sale-after-5-years) is SINGLE-scenario, reuses existing arrays
`RoiResult.exit_scenario` (optional, omitted-not-null when `purchase<=0`) is built in `index.ts` from the already-computed `projection` (last = cumulative 5y net) and `depreciation` (last = year-5 value) — never recalculated. It reflects ONLY the one occupancy posture the calc was run for.
**Why:** owner explicitly rejected showing 3 scenarios (conservative/realistic/optimistic) — the engine computes ONE scenario per call, and owner wanted the exit block to match whichever posture the user picked, with zero recalculation.
**How to apply:** `exit_result_eur = (cum5yNet + year5Value) − purchase`; the UI "Total return" row shows `exit_result_eur` (net), NOT gross `total_return_eur`, because the visible column subtracts purchase price as a line item, so it must reconcile to net. `total_loan_paid_eur = round(loanAnnual*5)` (== monthly×60) only when `loanAnnual>0`, else null. Green/red coloring lives on the result screen and is fine there — the "palette-only, no green/red" rule applies ONLY to the adaptive PDF document design system.
