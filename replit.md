...
- **Document Design System V1** (May 31): unified shared design system across adaptive PDFs for **valuation_report + proposal ONLY** (survey/ROI/charter/listing/DOCX/legacy untouched; opt-in via `exportSettings.engine:"adaptive"`). Embedded fonts (`core/fonts.generated.ts` base64 Gilroy 400/600/700/800 + Wix 400/500 → `FONT_FACE_CSS`; ~564KB, import-only). `core/theme.ts` full rewrite to ONE of each primitive: one `.tbl` (navy header + white text + zebra `rowAlt`), one `.val-box` metric card (mid = navy emphasis, `.val-amount` 19px/mid 21px so prices fit a half-width 2-col card), one `.kv-grid` paired key/value grid. Palette navy/gold/white/grey only — status chips palette-only (gold-ink positive, grey negative; NO green/red). `.page` min-height 261mm + abs `.page-footer` (Brand · Confidential · Page X of Y · Date); footer confidential label reuses localized `meta.watermarkText` (6 langs, shown only when confidential). `KeyValueGridNode.layout:"pairs"` (measure = `ceil(rows/2)*KV_ROW_MM`) for specs + accommodation; equipment = 2-col atomic categories; proposal gains photo gallery. Cover = full-bleed photo or `.cover-frame` premium-text fallback. Verified via probe: valuation 3pp, proposal 5pp, no near-empty/overflow pages, fonts/footers/grids/navy tables/palette chips correct. Cyrillic unsupported by Gilroy/Wix → Russian falls back to system font (deferred). Additive only; no Supabase/calc/route change.
