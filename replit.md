...
- **Stage 3 (next):** ROI calculator engine — серверный движок расчёта (revenue по market_rates × occupancy × season; expenses по expense_rates × management_style; чистый ROI / payback / IRR), снимет 501 с `POST /roi/calculate`. Seed market_rates + expense_rates перед этим.
